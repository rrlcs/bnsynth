...
# Architecture 1
class GCLN_Arch1(torch.nn.Module):
    literal_pairs = []

    def __init__(self, input_size, num_of_output_var, K, device):
        super(GCLN_Arch1, self).__init__()
        self.device = device
        self.K = K
...
class GCLN_Arch2(torch.nn.Module):
    literal_pairs = []

    def __init__(self, input_size, num_of_output_var, K, device):
        super(GCLN_Arch2, self).__init__()
        self.device = device
        self.K = K
        self.input_size = input_size
        self.output_size = num_of_output_var
    ...
    def forward(self, x):
        ...
        gated_or_res = torch.add(gated_or_res, 1.0 - self.layer_and_weights, alpha=1)
        # gated_or_res = self.apply_bias(gated_or_res, self.b2)
...
class GCLN_Arch3(torch.nn.Module):
    literal_pairs = []

    def __init__(self, input_size, num_of_output_var, K, device):
        super(GCLN_Arch3, self).__init__()
        self.device = device
        self.K = K
        self.input_size = input_size
        self.output_size = num_of_output_var
    ...
    def forward(self, x):
        ...
        for i in range(0, self.output_size):
            gated_inputs.append(self.apply_gates(self.layer_or_weights[:, i*self.K:(i+1)*self.K], inputs))
        gated_inputs = torch.stack(gated_inputs)
        # gated_inputs = self.apply_bias(gated_inputs, self.b1)
