...
class GCLN_CNF_Arch1(torch.nn.Module):
    ...
    def __init__(self, input_size, num_of_output_var, K, device):
        ...
        self.linear_layer_first = nn.Linear(self.input_size, 1024, bias=False)
        self.linear_layer_1 = nn.Linear(1024, 1024, bias=False)
        self.linear_layer_2 = nn.Linear(1024, 1024, bias=False)
        self.linear_layer_3 = nn.Linear(1024, 1024, bias=False)
        self.linear_layer_final = nn.Linear(1024, self.input_size, bias=False)
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            # self.linear_layer_first.weight.data.clamp_(0.0, 1.0)
            # self.linear_layer_1.weight.data.clamp_(0.0, 1.0)
            # self.linear_layer_2.weight.data.clamp_(0.0, 1.0)
            # self.linear_layer_3.weight.data.clamp_(0.0, 1.0)
            # self.linear_layer_final.weight.data.clamp_(0.0, 1.0)
        ...
        # inputs = self.linear_layer_first(inputs)  # .unsqueeze(-1)
        # inputs = self.linear_layer_1(inputs)
        # inputs = self.linear_layer_2(inputs)
        # inputs = self.linear_layer_3(inputs)
        # inputs = self.linear_layer_final(inputs).unsqueeze(-1)
...
class GCLN_CNF_Arch3(torch.nn.Module):
    ...
    def __init__(self, input_size, num_of_output_var, K, device):
        ...
        self.linear_layer_3 = nn.Linear(1024, 1024, bias=False)
        self.linear_layer_final = nn.Linear(1024, self.input_size, bias=False)
        # self.linear_layer_first.weight.data.uniform_(0.0, 1.0)
        # self.linear_layer_1.weight.data.uniform_(0.0, 1.0)
        # self.linear_layer_2.weight.data.uniform_(0.0, 1.0)
        # self.linear_layer_3.weight.data.uniform_(0.0, 1.0)
        # self.linear_layer_final.weight.data.uniform_(0.0, 1.0)
