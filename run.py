...
def prepare_ce(io_dict, counter_examples, num_of_vars, num_of_outputs):
	...
	out_var = np.array([0, 1])
	out_var = np.repeat(
		out_var.reshape(
			(
				out_var.size, num_of_outputs
				)
			), noce, axis=1
		).T.reshape((-1, 1))
...
def generate_counter_examples(
	...
	ce = torch.cat(
		[
			util.add_noise((ce.T[:, res >= 0.5]).T) for _ in range(n)
			]
		).double()
	return ce
...
def ce_train_loop(
		training_samples, io_dict, result, model, num_of_vars, num_of_outputs
	):
	loop = 0
	print("ce loop: ", result)
	while result==False and loop < 50:
		loop += 1
		print("Counter Example Loop: ", loop)
		ce = generate_counter_examples(
			args.no_of_samples, io_dict, model, py_spec, 
		...
		flag = 0
		# args.epochs += 5
		gcln, train_loss, valid_loss = tr.train_regressor(
			train_loader, validation_loader, loss_fn, args.learning_rate,
			args.epochs, input_size, num_of_outputs, output_var_idx, args.K, 
			device, args.P, flag, checkpoint=None
			)
...
if __name__ == "__main__":
	...
	# load data
	train_loader = dataLoader(training_set, training_size, args.P, input_var_idx, output_var_idx, num_of_outputs, args.threshold, args.batch_size)
	validation_loader = dataLoader(validation_set, training_size, args.P, input_var_idx, output_var_idx, num_of_outputs, args.threshold, args.batch_size)
	...
	if args.P == 0:
		...
		if not result:
			...
			result = ce_train_loop(training_samples, io_dict, result, counter_examples, num_of_vars, num_of_outputs)
		end_time = time.time()
